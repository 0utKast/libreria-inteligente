...
@patch("os.getenv", return_value="1234")
@patch("google.generativeai.configure")
def test_configure_genai_numeric_api_key(mock_configure,mock_getenv):
    configure_genai()
    mock_configure.assert_called_once_with(api_key="1234")

@patch("os.getenv", return_value = "test_api_key\n")
@patch("google.generativeai.configure")
def test_configure_genai_api_key_with_newline(mock_configure, mock_getenv):
    configure_genai()
    mock_configure.assert_called_once_with(api_key="test_api_key\n")
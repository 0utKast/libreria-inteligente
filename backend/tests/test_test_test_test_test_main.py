...
async def test_analyze_with_gemini_empty_input(mock_genai):
    mock_model = AsyncMock()
    mock_model.generate_content_async.return_value.text = '{"title": "", "author": "", "category": ""}'
    mock_genai.return_value = mock_model
...
@pytest.mark.asyncio
@patch("backend.main.genai.GenerativeModel")
async def test_analyze_with_gemini_invalid_json(mock_genai):
    mock_model = AsyncMock()
    mock_model.generate_content_async.return_value.text = "invalid json"
    mock_genai.return_value = mock_model
    result = await analyze_with_gemini("test text")
    assert result == {"title": "Error de IA", "author": "Error de IA", "category": "Error de IA"}
...
@patch("backend.main.fitz.open")
def test_process_pdf_error(mock_fitz_open):
...
def test_process_epub_success(mock_read_epub):
    ...
    mock_book.get_items_of_type.return_value = [mock_item, mock_item] #Simula que hay imagen de portada
...
@patch("backend.main.epub.read_epub")
def test_process_epub_error(mock_read_epub):
    mock_read_epub.side_effect = FileNotFoundError
    with pytest.raises(FileNotFoundError):
...
@patch("backend.main.zipfile.ZipFile")
@patch("backend.main.tempfile.TemporaryDirectory")
async def test_convert_epub_to_pdf_error(mock_tempdir, mock_zipfile, mock_css, mock_html):
    mock_tempdir.return_value.__enter__.return_value = "test_dir"
    mock_zipfile.return_value.__enter__.return_value.extractall = MagicMock(side_effect=Exception("Zip error"))
    mock_file = MagicMock(spec=UploadFile)
    mock_file.filename = "test.epub"
    mock_file.read = AsyncMock(return_value=b"test data")
...
async def test_upload_book_for_rag_success(mock_process_book, mock_db):
    ...
    mock_file.filename = "test.pdf"
    mock_file.read = AsyncMock(return_value=b"test data")
    mock_process_book.return_value = {"book_id":1}
...
@pytest.mark.asyncio
@patch("backend.main.rag.query_rag")
async def test_query_rag_endpoint_empty_query(mock_query_rag):
    mock_query_rag.return_value = "Test response"
    query_data = {"query": "", "book_id": "test_id"}
...
@pytest.mark.asyncio
@patch("backend.main.rag.query_rag")
async def test_query_rag_endpoint_missing_book_id(mock_query_rag):
    mock_query_rag.return_value = "Test response"

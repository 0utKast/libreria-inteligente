...
@patch('backend.main.genai')
async def test_analyze_with_gemini_empty_text(mock_genai):
    mock_genai.GenerativeModel.side_effect = Exception("Gemini API Error")
...
def test_process_epub_error():
    with patch('backend.main.epub.read_epub', side_effect=FileNotFoundError("File not found")):
        with pytest.raises(FileNotFoundError):
...
async def test_upload_book_invalid_file():
    mock_file = MagicMock(spec=UploadFile, filename="test.txt", file=io.BytesIO(b"test"))
...
async def test_convert_epub_to_pdf_error():
    mock_file = MagicMock(spec=UploadFile, filename="test.epub", file=io.BytesIO(b"test"))
    with patch('backend.main.HTML', side_effect=Exception("HTML Error")):
...
async def test_query_rag_endpoint_error():
    mock_query_data = schemas.RagQuery(query="test query", book_id="test book id")
    with patch('backend.main.rag.query_rag', side_effect=Exception("RAG Error")):
        with pytest.raises(Exception):

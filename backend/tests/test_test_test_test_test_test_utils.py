...
@patch('os.getenv', side_effect=lambda key: 123 if key == "GOOGLE_API_KEY" else None)
@patch('google.generativeai.configure')
def test_configure_genai_numeric_api_key(mock_configure, mock_getenv):
    with pytest.raises(TypeError) as excinfo:
        configure_genai()
    assert "Invalid API key type" in str(excinfo.value)

@patch('os.getenv', side_effect=lambda key: {'api_key': 'test-api-key'} if key == "GOOGLE_API_KEY" else None)
@patch('google.generativeai.configure')
def test_configure_genai_dict_api_key(mock_configure, mock_getenv):
    with pytest.raises(TypeError) as excinfo:
        configure_genai()
    assert "Invalid API key type" in str(excinfo.value)
...
@patch('os.getenv', side_effect=lambda key: ['test-api-key'] if key == "GOOGLE_API_KEY" else None)
@patch('google.generativeai.configure')
def test_configure_genai_list_api_key(mock_configure, mock_getenv):
    with pytest.raises(TypeError) as excinfo:
        configure_genai()
    assert "Invalid API key type" in str(excinfo.value)
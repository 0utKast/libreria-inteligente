...
def test_process_pdf_empty_file():
    ...
    with patch("backend.main.fitz.open") as mock_open:
        mock_open.side_effect = Exception("Empty PDF file")
...
def test_upload_book_invalid_file_type():
    mock_file = MagicMock(spec=UploadFile)
    mock_file.filename = "test.txt"
    mock_file.file = io.BytesIO(b"test")
    with patch('backend.main.database.SessionLocal') as mock_session:
        with pytest.raises(HTTPException) as excinfo:
            upload_book(db=mock_session.return_value, book_file=mock_file)
...
@patch("backend.main.crud.get_book")
def test_download_book(mock_get_book, mock_fileresp):
    mock_book = MagicMock()
...
@patch("backend.main.FileResponse")
@patch("backend.main.crud.get_book")
def test_download_book_not_found(mock_get_book, mock_fileresp):
...
@pytest.mark.asyncio
@patch("backend.main.convert_epub_to_pdf")
async def test_convert_epub_to_pdf_failure(mock_convert):
...
async def test_upload_book_for_rag(mock_process_book):
    ...
    mock_file.filename = "test.epub"
    mock_file.read = AsyncMock(return_value=b"test")
    mock_process_book.return_value = {"book_id": 1, "message": "Success"}
...
async def test_upload_book_for_rag_failure(mock_process_book):
    mock_process_book.side_effect = Exception("RAG processing failed")
    mock_file = MagicMock(spec=UploadFile)
    mock_file.filename = "test.epub"
    mock_file.read = AsyncMock(return_value=b"test")
    with pytest.raises(Exception) as excinfo:
        await upload_book_for_rag(file=mock_file)
    assert "RAG processing failed" in str(excinfo.value)
...
async def test_query_rag_endpoint_failure(mock_query_rag):
    mock_query_rag.side_effect = Exception("RAG query failed")
    query_data = schemas.RagQuery(query="Test query", book_id="test_id")
    with pytest.raises(Exception) as excinfo:
        await query_rag_endpoint(query_data=query_data)
    assert "RAG query failed" in str(excinfo.value)

...
describe('Header Component', () => {
  ...
  it('handles fetch failure gracefully', async () => {
    ...
    expect(screen.getByText(/0 libros en la biblioteca/i)).toBeInTheDocument();

  });


  it('displays correct number of books after successful fetch', async () => {
    mockFetch.mockImplementation(() => 
      Promise.resolve({
        ok: true,
  ...
  it('displays zero books if fetch returns zero', async () => {
    mockFetch.mockImplementation(() => 
      Promise.resolve({
        ok: true,
  ...
  it('handles non-number count from API', async () => {
      mockFetch.mockImplementation(() =>
        Promise.resolve({
          ok: true,
  ...
  it('shows loading indicator while fetching', async () => {
    mockFetch.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve({ ok: true, json: () => Promise.resolve({ count: 10 }) }), 500)));
    render(<BrowserRouter><Header /></BrowserRouter>);
    expect(screen.getByText(/Cargando.../i)).toBeInTheDocument();
    await screen.findByText(/10 libros en la biblioteca/i);
